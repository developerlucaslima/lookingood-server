...
import type { Booking, Prisma, Status } from '@prisma/client'
...
export class InMemoryBookingsRepository implements BookingsRepository {
	...
	async isBookingConflict(professionalId: string, startTime: Date, endTime: Date) {
		let conflict = false
		for (const booking of this.items.values()) {
			const isSameProfessional = booking.professionalId === professionalId
			const isStartTimeOverlap = startTime >= booking.startTime && startTime < booking.endTime
			const isEndTimeOverlap = endTime > booking.startTime && endTime <= booking.endTime
			const isIntervalOverlap = startTime <= booking.startTime && endTime >= booking.endTime
			if (isSameProfessional && (isStartTimeOverlap || isEndTimeOverlap || isIntervalOverlap)) {
				conflict = true
			}
		}
