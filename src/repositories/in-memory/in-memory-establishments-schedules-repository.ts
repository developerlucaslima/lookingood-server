...
import type { EstablishmentSchedule, Prisma } from '@prisma/client'
import type { EstablishmentsSchedulesRepository } from '../establishments-schedules-repository'

export class InMemoryEstablishmentsSchedulesRepository
	implements EstablishmentsSchedulesRepository
{
	public items: Map<EstablishmentSchedule['id'], EstablishmentSchedule> = new Map()
	...
	async findByEstablishmentIdAndWeekDay(establishmentId: string, weekDay: string) {
		const schedules = Array.from(this.items.values()).flat()

		return (
			schedules.find(
				(schedule) => schedule.establishmentId === establishmentId && schedule.weekDay === weekDay,
			) || null
		)
	}
}
