...
import type { Prisma, ProfessionalSchedule } from '@prisma/client'
import type { ProfessionalsSchedulesRepository } from '../professionals-schedules-repository'

export class InMemoryProfessionalsSchedulesRepository implements ProfessionalsSchedulesRepository {
	public items: Map<ProfessionalSchedule['id'], ProfessionalSchedule> = new Map()
	...
	async findByProfessionalIdAndWeekDay(professionalId: string, weekDay: string) {
		const schedules = Array.from(this.items.values()).flat()
		return (
			schedules.find(
				(schedule) => schedule.professionalId === professionalId && schedule.weekDay === weekDay,
			) || null
		)
	}
}
