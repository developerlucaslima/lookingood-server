import type { ProfessionalSchedule } from '@prisma/client'
...
export function isBookingWithinProfessionalSchedule(
	...
	const professionalStartMinutes = getMinutesByTime(professionalSchedule.startTime)
	const professionalEndMinutes = professionalStartMinutes + professionalSchedule.minutesWorking
	const bookingStartMinutes = getMinutesByTime(`${startTime.getHours()}:${startTime.getMinutes()}`)
	const bookingEndMinutes = getMinutesByTime(`${endTime.getHours()}:${endTime.getMinutes()}`)
	...
	if (professionalSchedule.breakTime && professionalSchedule.minutesBreak) {
		const professionalBreakStartMinutes = getMinutesByTime(professionalSchedule.breakTime)
