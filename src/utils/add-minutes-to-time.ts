export function addMinutesToTime(startTime: string, minutesToAdd: number): string {
	...
	const hours = Number.parseInt(hoursStr, 10) // Parsing the hours string into an integer
	const minutes = Number.parseInt(minutesStr, 10) // Parsing the minutes string into an integer
	...
	const newMinutesStr = remainingMinutes < 10 ? `0${remainingMinutes}` : `${remainingMinutes}` // Formatting remaining minutes with leading zero if necessary
