...
import type { ProfessionalsSchedulesRepository } from '@/repositories/professionals-schedules-repository'
...
export class BookingServiceUseCase {
	...
	async execute({
		...
		if (!professional) throw new ProfessionalNotFoundException()
		...
		if (!establishment) throw new EstablishmentNotFoundException()
		...
		if (!professionalSchedule) throw new ProfessionalNotAvailableException()

		if (!isBookingWithinProfessionalSchedule(professionalSchedule, startTime, endTime)) {
	...
	private async verifyResources({
		...
		return { professional, service, user, establishment }
	}
}
