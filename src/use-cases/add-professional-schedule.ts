import type { $Enums, ProfessionalSchedule } from '@prisma/client'
...
export class AddProfessionalScheduleUseCase {
	...
	async execute({
		...
		const establishment = await this.establishmentsRepository.findById(professional.establishmentId)
		if (!establishment) {
